...
xdescribe('Transaction List', function () {
  let wrapper
  ...
  const props = {
    ...
    fetchGasEstimates: sinon.stub().resolves({ blockTime }),
  ...
  it('renders and calls fetchBasicGasAndTimeEstimates/fetchGasEstimates for blockTime on mount', function () {
    console.log(wrapper.find('TransactionList').props())
    setImmediate(() => {
      // console.log(wrapper.find('TransactionList').prop('fetchBasicGasAndTimeEstimates').callCount)
