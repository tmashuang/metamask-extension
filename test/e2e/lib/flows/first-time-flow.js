...
export async function firstTimeFlow (page, path, skipChallenge = false) {
  ...
  switch (path) {
    ...
    case 'create':
      ...
      await page.click('button.btn-default.page-container__footer-button')
    ...
    case 'import':
      ...
      // MetaMetrics
      await page.waitFor('.metametrics-opt-in')
      await page.click('button.btn-primary.page-container__footer-button')
...
async function clickWord (page, word) {
  const xpath = `//div[contains(@class, 'confirm-seed-phrase__seed-word--sorted') and not(contains(@class, 'confirm-seed-phrase__seed-word--selected')) and contains(text(), '${word}')]`

  const wordButton = await page.$x(xpath)
  await wordButton[0].click('button.first-time-flow__button')
}

export async function confirmSeedPhase (page, skipChallenge) {
  // Reveal Seed Phrase
